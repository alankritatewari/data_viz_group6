...
plot_goals <- 
  ...
  scale_x_continuous(breaks = 
   ...
   labs(x = "League", y = "Average Goals per Game", 
       title = "The Bundesliga Stands Out: Leading in Average Goals per Game",
...
data %>% 
  ...
  # Custom colors
  scale_fill_manual(values=c('#bebebe', '#eda247')) +
  
  # Custom titles and axes
  labs(title = "Seria A and La Liga have more yellow cards",
...
data %>% 
  ...
  # Custom titles and axes
  labs(title = "Seria A and Ligue 1 have more red cards",
...
data %>% 
  ...
  # Custom titles and axes
  labs(title = "Seria A and La Liga are more 'dangerous'",
...
ggplot(top_5_teams, aes(x = percentage_goals, y = fct_reorder(name.y, percentage_goals), fill = as.factor(name.x))) +
  ...
  geom_text(aes(label = sprintf("%.2f%%", percentage_goals), group = name.x), 
            position = position_nudge(x = 1.0), size = 3) +
  ...
  labs(title = "Bayern Munich and Borussia Dortmund collectively score 17.92% goals for Bundesliga, making it a top choice", caption = " Top 5 Teams with Highest Share of Goals Scored in Each League",
       x = "Percentage of Goals Scored (Within League)", y = "Team Names") +
  theme_minimal() + 
  scale_x_continuous() +
  theme(
...
plot_keyp <- ggplot(pct_key_passes, aes(x = season, y = ratio, group = teamID, 
  ...
  geom_text(aes(label = round(ratio, 2)),    
            ...
            color = "black",
            size=2.5)+
   labs(title = "Synced Trajectories: Bayern Munich and Borussia Dortmund Match Playmaking Trends",
...
data %>% 
  ...
  # Custom colors
  scale_fill_manual(values=c('#c70007', '#FDE100')) +
...
plot_dortmund <- ggplot(top_scorer, aes(x = year, y = count.x, color = name_dortmund))+
  ...
  labs(y = "Number of Goals",
       ...
       title = "Dortmund has more tactical diversity allowing for different strikers to shine",
...
ggplot(shots_three_players, aes(x = name, y = shotResult, fill = percent)) + 
  ...
  scale_fill_gradient(low = "white", high = "#FDE100") +
  ...
  labs(title = "Haaland is a scoring machine, Sancho is a balanced player & Marco is weak",
...
ggplot(assist_three_players, aes(x = assisterID, y = shotResult, fill = percent)) + 
  ...
  labs(title = "Erling Haaland sets the pass in a better position that other two players",
